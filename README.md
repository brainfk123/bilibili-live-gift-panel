...
Release job 绑定受保护的 GitHub Environment `release`；应为它配置必要的审批/分支规则。stable 只接受 GitHub Actions variables `UPDATE_API_BASE_URL`、`EVSIGN_CERTIFICATE`、`EVSIGN_PUBLISHER_IDENTITY` 和受保护的 `RELEASE_TOOLING_COMMIT_SHA`，以及 secrets `EVSIGN_KEY`、`EVSIGN_PASSWORD`。`EVSIGN_PUBLISHER_IDENTITY` 必须是精确结构化 NaisNet 身份 `{"country":"CN","organization":"NaisNet Technology Co., Ltd.","organizationId":"91210103MA7CJ3C094"}`；不再接受自由格式 Subject 或旧 profile/legacy fallback。bridge 使用独立的 `EVSIGN_BRIDGE_CERTIFICATE`、`EVSIGN_BRIDGE_PUBLISHER_IDENTITY` 与 bridge secrets，且只能由专用 v0.4.11 workflow 使用。证书校验解析 DER 中唯一的 C/O/Subject serialNumber 和 Code Signing EKU，不依赖显示 Subject、叶证书顺序或 thumbprint。COS 凭证只安装在 Lighthouse 的独立镜像服务环境中，不进入 GitHub Actions。部署变量与镜像服务说明见[国内更新 API 部署说明](deploy/update-api/README.md)，bridge 的审批输入见专用 runbook，避免在此复制 secrets。
