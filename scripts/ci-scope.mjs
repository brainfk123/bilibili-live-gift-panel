...
const skipPatterns = [/^(?:docs\/|acceptance\/|.*\.md$)/, /^\.dockerignore$/, /^(?:src\/hosted\/|goserver\/(?:cmd\/hosted|internal\/hosted)\/|deploy\/hosted\/)/, /^tests\/hosted-.*\.(?:test|contract)\.ts$/, /^tests\/Dockerfile\.hosted-deploy$/, /^tests\/(?:development-workflow|ui-parity-requirements|exe-retirement-contract)\.test\.ts$/, /^scripts\/(?:build-hosted|test-hosted-mysql)\.mjs$/, /^(?:hosted|obs)\.html$/, /^vite\.hosted\.config\.ts$/];
...
export function formatGitHubSummary(decision) {
  ...
  const rows = decision.reasons.map(({ path, level, reason }) =>
    `| ${escapeMarkdownCell(decision.windowsLevel)} | ${escapeMarkdownCell(decision.runWindows)} | ${escapeMarkdownCell(decision.runMySQL)} | ${escapeMarkdownPathCell(path)} | ${escapeMarkdownCell(level)} | ${escapeMarkdownCell(reason)} |`);
...
function escapeMarkdownPathCell(value) {
  let escaped = '';
  for (const character of String(value)) {
    escaped += `&#${character.codePointAt(0)};`;
  }
  return escaped;
}
