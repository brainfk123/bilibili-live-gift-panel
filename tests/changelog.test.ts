...
describe('versioned changelog', () => {
  it('keeps the latest release first and resolves v-prefixed versions', () => {
    ...
    expect(normalizeChangelogVersion(' v0.2.1 ')).toBe('0.2.1');
    expect(changelogReleaseForVersion('v0.2.1')?.title).toBe('礼物变体与实战教程');
  });

  it('opens once for a known installed version and ignores development builds', () => {
    expect(shouldShowChangelog('0.2.1', '')).toBe(true);
    expect(shouldShowChangelog('v0.2.1', '0.2.1')).toBe(false);
  ...
  it('merges hosted history over the bundled current-release fallback', () => {
    ...
    expect(merged.map((release) => release.version)).toEqual(['0.2.1', '0.1.0']);
