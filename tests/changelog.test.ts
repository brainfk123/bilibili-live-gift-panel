...
describe('versioned changelog', () => {
  ...
  it('bundles only the v0.4.8 release with online migration export', () => {
    expect(CHANGELOG_RELEASES).toHaveLength(1);
    expect(latestChangelogRelease()).toBe(CHANGELOG_RELEASES[0]);
    expect(normalizeChangelogVersion(' v0.4.8 ')).toBe('0.4.8');
    expect(changelogReleaseForVersion('v0.4.8')).toEqual({
      version: '0.4.8',
      date: '2026-08-29',
      title: '新增在线迁移导出',
  ...
  it('opens once for a known installed version and ignores development builds', () => {
    expect(shouldShowChangelog('0.4.8', '')).toBe(true);
    expect(shouldShowChangelog('v0.4.8', '0.4.8')).toBe(false);
  ...
  it('merges hosted history over the bundled current-release fallback', () => {
    ...
    expect(merged.map((release) => release.version)).toEqual(['0.4.8', '0.1.0']);
  ...
  it('ignores the removed training visual from bundled and hosted changelogs', () => {
    expect(changelogReleaseForVersion('0.4.8')?.visuals).toEqual([]);
    const [hosted] = normalizeChangelogReleases({
      releases: [{
