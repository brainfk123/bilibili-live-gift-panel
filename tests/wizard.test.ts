...
describe('single-page configuration rendering', () => {
  ...
  it('keeps tutorial spotlights above template and attribute workspaces', () => {
    ...
    expect(configCss).toMatch(/\.config-root \.tour-target-flow-segment \{[\s\S]*?stroke-dasharray: 12 88;[\s\S]*?animation: tour-outline-flow 1\.8s linear infinite;/);
  ...
  it('opens a visual changelog manually and remembers the latest viewed version', async () => {
    ...
    expect(textOf(dialog!)).toContain('缩小安装包并简化安全发布');
    expect(textOf(dialog!)).toContain('完整保留 v0.4.12 的礼物兼容、安全更新与内嵌 FFmpeg 功能');
    ...
    await vi.waitFor(() => expect(loadState().settings.lastSeenChangelogVersion).toBe('0.4.13'));
    ...
    const fetchMock = vi.fn(async (input: string | URL | Request) => {
      ...
      if (url === '/api/update') {
        return Response.json({
          code: 0,
          update: {
            state: 'up-to-date', currentVersion: '0.4.13', latestVersion: '0.4.13',
  ...
  it('shows one spotlight bubble over the next key UI', () => {
    ...
    expect(root.querySelector('.tour-target-flow-tail')).not.toBeNull();
    expect(root.querySelector('.tour-target-flow-body')).not.toBeNull();
