...
describe('single-page configuration rendering', () => {
  ...
  it('opens a visual changelog manually and remembers the latest viewed version', async () => {
    ...
    expect(textOf(dialog!)).toContain('响应大航海和醒目留言');
    ...
    await vi.waitFor(() => expect(loadState().settings.lastSeenChangelogVersion).toBe('0.2.2'));
    ...
    const fetchMock = vi.fn(async (input: string | URL | Request) => {
      ...
      if (url === '/api/update') {
        return Response.json({
          code: 0,
          update: {
            state: 'up-to-date', currentVersion: '0.2.2', latestVersion: '0.2.2',
...
describe('OBS attribute display', () => {
  ...
  it('keeps KPI gift choices at their content height inside the scrolling editor', () => {
    ...
    expect(configCss).toMatch(/\.gift-kpi-card-visual\s*\{[^}]*translateZ\(var\(--card-visual-depth\)\)/);
