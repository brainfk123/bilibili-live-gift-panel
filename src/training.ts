...
export const MAIN_LESSON_DETAILS: Record<TutorialLesson, MainLessonDetail> = {
  ...
  attribute: {
    steps: ['找到“属性与礼物规则”区域。', '点击“添加属性”。', '在创建中心查看模板与空白创建两种入口。'],
    outcome: '创建中心负责选择起点；玩法模板适合快速开播，空白创建适合完整自定义。',
  },
  template: {
    steps: ['了解玩法模板会自动创建属性、规则和输出。', '本次训练选择“从空白创建”。', '进入包含四个工作区的属性工作台。'],
    outcome: '你会亲手完成一台加班机；学会后也可以直接使用玩法模板快速创建。',
  },
  basics: {
    steps: ['使用工作台里的加班机训练模板。', '确认名称、起始值和计时器显示格式。', '观察下方 OBS 数值预览。'],
  ...
  timer: {
    steps: ['添加一个定时器。', '设置每 1 秒执行，并在加班时间大于 0 时减 1。', '模拟一次并观察条件是否满足。'],
    outcome: '定时器由托盘后台运行，不需要 OBS 页面保持打开，也不会显示为礼物卡片。',
  },
  appearance: {
    steps: ['进入“输出与预览”工作区。', '检查默认播报、状态映射和当前属性皮肤。', '点击“确认输出预览”。'],
    outcome: '这些设置只改变 OBS 画面；属性当前值、礼物规则和定时器仍由后台管理。',
  },
  preset: {
  ...
  enable: {
    steps: ['保存后回到属性列表。', '悬停或键盘聚焦属性卡片，等待详情展开。', '打开礼物规则或定时器右侧的开关。'],
    outcome: '小卡片只显示关键信息；展开详情可以编辑、删除、复制链接和控制后台启用状态。',
  },
  output: {
    steps: ['在展开的属性卡片中找到专属链接。', '点击“复制 OBS 链接”。', '在 OBS 中添加“浏览器”来源，确认画面后即可关闭配置页。'],
    outcome: '配置页负责设置，托盘后台负责收礼和计算，OBS 链接只负责显示；关闭配置页不会停服。',
  },
};
