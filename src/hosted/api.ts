...
export interface MigrationPreview {
  ...
  generalSettingsCompatibility: MigrationCompatibility;
  blindBoxDisplayCompatibility?: MigrationCompatibility;
  canConfirm: boolean;
}
...
export interface MigrationJob {
  ...
  obsReissueAvailable?: boolean;
}
export interface MigrationHistoryJob { id: number; status: MigrationJob['status']; createdAt: string; appliedAt?: string; expiresAt?: string; rollbackExpiresAt?: string; obsReissueAvailable?: boolean }
...
function globalAppearance(value: unknown): boolean {
  const item = object(value); return item !== undefined && exactKeys(item, ['theme', 'fontSize', 'accentColor', 'align', 'panelOpacity', 'showConnection']) && (item.theme === 'dark' || item.theme === 'light') && appearanceScalars(item);
}
function displayAppearance(value: unknown): boolean {
  const item = object(value); return item !== undefined && exactKeys(item, ['themeId', 'fontSize', 'accentColor', 'showConnection', 'align', 'panelOpacity']) && ['minimal', 'glass', 'rpg', 'pixel', 'neon', 'kawaii'].includes(String(item.themeId)) && appearanceScalars(item);
}
function display(value: unknown): boolean {
  const item = object(value); return item !== undefined && exactKeys(item, ['variant'], ['themeId', 'appearance', 'title', 'min', 'max', 'lowThreshold', 'leftLabel', 'rightLabel', 'valueMappings']) && string(item.variant) && optional(item.themeId, string) && optional(item.appearance, displayAppearance) && optional(item.title, string) && optional(item.min, finite) && optional(item.max, finite) && optional(item.lowThreshold, finite) && optional(item.leftLabel, string) && optional(item.rightLabel, string) && optional(item.valueMappings, (items) => arrayOf(items, (entry) => { const mapping = object(entry); return mapping !== undefined && exactKeys(mapping, ['value', 'label'], ['color']) && finite(mapping.value) && string(mapping.label) && optional(mapping.color, string); }));
}
...
function definition(value: unknown): value is HostedConfigurationDefinition {
  const item = object(value);
  if (!item || !exactKeys(item, ['attributes', 'displayScenes', 'giftTargetPanels', 'activities', 'rules', 'timerRules', 'formulaPresets', 'gifts'], ['appearance', 'blindBoxDisplay', 'generalSettings', 'cropPresets', 'migrationHash', 'simplePlay']) || !arrayOf(item.attributes, attributeDefinition)) return false;
  if (!optional(item.appearance, globalAppearance) || !optional(item.blindBoxDisplay, displayAppearance)) return false;
  if (!optional(item.generalSettings, (value) => { const settings = object(value); return settings !== undefined && exactKeys(settings, ['configurationMode']) && (settings.configurationMode === 'simple' || settings.configurationMode === 'advanced'); })) return false;
  if (!optional(item.cropPresets, (value) => arrayOf(value, (entry) => { const preset = object(entry); const crop = preset && object(preset.crop); return preset !== undefined && exactKeys(preset, ['id', 'crop']) && string(preset.id) && crop !== undefined && exactKeys(crop, ['x', 'y', 'width', 'height']) && finite(crop.x) && finite(crop.y) && finite(crop.width) && finite(crop.height) && crop.x >= 0 && crop.y >= 0 && crop.width > 0 && crop.height > 0 && crop.x + crop.width <= 1 && crop.y + crop.height <= 1; }))) return false;
  if (!optional(item.migrationHash, (value) => typeof value === 'string' && /^[0-9a-f]{64}$/.test(value))) return false;
  const scene = (entry: unknown): boolean => { const row = object(entry); return row !== undefined && exactKeys(row, ['id', 'name', 'attributeIds', 'layout', 'themeId'], ['appearance']) && string(row.id) && text(row.name) && strings(row.attributeIds) && text(row.layout) && text(row.themeId) && optional(row.appearance, displayAppearance); };
  const panel = (entry: unknown): boolean => { const row = object(entry); return row !== undefined && exactKeys(row, ['id', 'name', 'layout', 'items'], ['appearance']) && string(row.id) && text(row.name) && text(row.layout) && optional(row.appearance, displayAppearance) && arrayOf(row.items, (child) => { const item = object(child); return item !== undefined && exactKeys(item, ['giftId', 'target', 'barStyle'], ['name']) && number(item.giftId) && number(item.target) && text(item.barStyle) && optional(item.name, string); }); };
...
function migrationPreview(value: unknown): MigrationPreview | undefined {
  const item = object(value);
  const counts = migrationCounts(item?.counts); const source = migrationSource(item?.source); const selection = migrationSelection(item?.selection); const generalSettings = migrationGeneralSettings(item?.generalSettings); const generalSettingsCompatibility = migrationCompatibility(item?.generalSettingsCompatibility); const blindBoxDisplayCompatibility = item?.blindBoxDisplayCompatibility === undefined ? undefined : migrationCompatibility(item.blindBoxDisplayCompatibility);
  if (!item || !exactKeys(item, ['id', 'expiresAt', 'reused', 'counts', 'source', 'selection', 'generalSettings', 'generalSettingsCompatibility', 'canConfirm'], ['warnings', 'ignored', 'roomSuggestion', 'units', 'groups', 'conflicts', 'blindBoxDisplayCompatibility']) || !number(item.id) || item.id === 0 || !instant(item.expiresAt) || typeof item.reused !== 'boolean' || !counts || !source || !selection || !generalSettings || !generalSettingsCompatibility || (item.blindBoxDisplayCompatibility !== undefined && !blindBoxDisplayCompatibility) || typeof item.canConfirm !== 'boolean' || (item.warnings !== undefined && !nonEmptyStringArray(item.warnings)) || (item.ignored !== undefined && !nonEmptyStringArray(item.ignored)) || (item.roomSuggestion !== undefined && !string(item.roomSuggestion)) || (item.units !== undefined && !Array.isArray(item.units)) || (item.groups !== undefined && !Array.isArray(item.groups)) || (item.conflicts !== undefined && !Array.isArray(item.conflicts))) return undefined;
