...
export function createMigrationFlow(api: MigrationAPI, render: (state: MigrationViewState) => void, clock: { now(): Date } = { now: () => new Date() }) {
  ...
  const publish = (): void => { if (!disposed) render({ ...(preview ? { preview: structuredClone(preview) } : {}), ...(job ? { job: structuredClone(job) } : {}), ...(proof ? { proof: { qrImage: proof.qrImage, expiresAt: proof.expiresAt } } : {}), ...(proofStatus ? { proofStatus } : {}), replacementConfirmed, keepRoomSuggestion, rawFileActive, operationInFlight, canApply: canApply(), canCancel: canCancel(), canRefresh: canRefresh(), canRollback: canRollback(), ...(days() === undefined ? {} : { rollbackDaysRemaining: days() }), ...(error ? { error } : {}) }); };
  ...
  const checkedProof = async (started: number): Promise<string> => { const active = await nextProof(started); if (!api.pollLogin) throw new HostedAPIError('invalid_request', 400); const result = await api.pollLogin(active.challengeId); ensureLive(started); if (result.status === 'pending' || result.status === 'scanned') { proofStatus = result.status; error = result.status === 'pending' ? '等待 B 站扫码确认，请稍后重试' : '已扫码，请在手机确认'; publish(); throw new HostedAPIError('verification_pending', 202); } if (result.status !== 'verified') { discardProof(); throw new HostedAPIError('proof_rejected', 401); } proofStatus = undefined; return active.challengeId; };
  ...
  return Object.freeze({
    ...
    async dispose(): Promise<void> { generation += 1; disposed = true; const active = proof; proof = undefined; proofStatus = undefined; preview = undefined; job = undefined; error = undefined; rawFileActive = false; operationInFlight = false; bestEffortCancel(active); },
  });
}
