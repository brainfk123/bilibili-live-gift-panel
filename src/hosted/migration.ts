...
export function createMigrationFlow(api: MigrationAPI, render: (state: MigrationViewState) => void, clock: { now(): Date } = { now: () => new Date() }) {
  ...
  const runProofOperation = (action: (challengeID: string, started: number) => Promise<void>): Promise<void> => {
    if (proofOperation) return Promise.reject(operationConflict());
  ...
  return Object.freeze({
    async preview(file: MigrationFile): Promise<void> { if (proofOperation) throw operationConflict(); if (disposed || !api.previewMigration || !file.name.toLowerCase().endsWith('.json') || !Number.isSafeInteger(file.size) || file.size < 0 || file.size > migrationFileLimit) throw new HostedAPIError('invalid_request', 400); const started = ++generation; rawFileActive = true; publish(); let raw = ''; try { raw = await file.text(); ensureLive(started); const result = await api.previewMigration(raw); if (disposed || started !== generation) { if (!result.reused) bestEffortJobCancel(result.id); throw new HostedAPIError('operation_failed', 0); } const authoritativeJob = result.reused ? await api.getMigration?.(result.id) : undefined; ensureLive(started); if (result.reused && !authoritativeJob) throw new HostedAPIError('invalid_request', 400); preview = result; job = authoritativeJob ?? { id: result.id, status: 'previewed', expiresAt: result.expiresAt }; replacementConfirmed = false; keepRoomSuggestion = false; error = undefined; } catch (cause) { if (!disposed && started === generation) fail(); throw cause; } finally { raw = ''; if (!disposed && started === generation) { rawFileActive = false; publish(); } } },
    acceptPreview(value: MigrationPreview): void { if (disposed || proofOperation) return; generation += 1; preview = structuredClone(value); job = { id: value.id, status: 'previewed', expiresAt: value.expiresAt }; replacementConfirmed = false; keepRoomSuggestion = false; error = undefined; publish(); },
    confirmReplacement(value: boolean): void { if (disposed || proofOperation) return; replacementConfirmed = value; publish(); },
    setKeepRoomSuggestion(value: boolean): void { if (disposed || proofOperation) return; keepRoomSuggestion = value; publish(); },
    apply(): Promise<void> { if (proofOperation) return Promise.reject(operationConflict()); if (!preview || !canApply() || !api.applyMigration) return Promise.reject(new HostedAPIError('invalid_request', 400)); const id = preview.id; const keep = keepRoomSuggestion; return runProofOperation(async (challengeID, started) => { const result = await api.applyMigration!(id, challengeID, keep); ensureLive(started); job = result; discardProof(); error = undefined; publish(); }); },
    rollback(): Promise<void> { if (proofOperation) return Promise.reject(operationConflict()); if (!job || !canRollback() || !api.rollbackMigration) return Promise.reject(new HostedAPIError('invalid_request', 400)); const id = job.id; return runProofOperation(async (challengeID, started) => { const result = await api.rollbackMigration!(id, challengeID); ensureLive(started); job = result; discardProof(); error = undefined; publish(); }); },
    async refresh(id: number): Promise<void> { if (proofOperation) throw operationConflict(); if (disposed || !canRefresh() || !job || id !== job.id || !api.getMigration) throw new HostedAPIError('invalid_request', 400); const started = ++generation; try { const result = await api.getMigration(id); ensureLive(started); job = result; error = undefined; publish(); } catch (cause) { if (!disposed && started === generation) fail(); throw cause; } },
    async cancel(): Promise<void> { if (proofOperation) throw operationConflict(); if (disposed || !canCancel() || !job || !api.cancelMigration) throw new HostedAPIError('invalid_request', 400); const started = ++generation; const id = job.id; try { const result = await api.cancelMigration(id); ensureLive(started); job = result; error = undefined; publish(); } catch (cause) { if (!disposed && started === generation) fail(); throw cause; } },
...
export function mountMigrationView(root: HTMLElement, api: MigrationAPI, callbacks: { onConfiguration(): void }) {
  const document = root.ownerDocument; let disposed = false; let selected: File | undefined; let state: MigrationViewState = { replacementConfirmed: false, keepRoomSuggestion: false, rawFileActive: false, canApply: false, canCancel: false, canRefresh: false, canRollback: false };
  ...
  const keep = document.createElement('label'); const keepBox = document.createElement('input'); keepBox.type = 'checkbox'; keep.append(keepBox); keepBox.addEventListener('change', () => flow.setKeepRoomSuggestion(keepBox.checked));
  ...
  const render = (next: MigrationViewState): void => {
    if (disposed) return; state = next; const busy = next.rawFileActive || Boolean(next.proof);
    ...
    visible(groups, Boolean(next.preview)); visible(replace, Boolean(next.preview)); replaceBox.checked = next.replacementConfirmed; replaceBox.disabled = Boolean(next.proof);
    ...
    visible(keep, Boolean(next.preview?.roomSuggestion)); keep.textContent = next.preview?.roomSuggestion ? `保留建议直播间 ${next.preview.roomSuggestion}（不会自动运行）` : ''; keep.append(keepBox); keepBox.checked = next.keepRoomSuggestion; keepBox.disabled = Boolean(next.proof);
    visible(apply, next.canApply); apply.textContent = next.proof ? '检查 B 站扫码并应用' : '创建 B 站验证后应用'; apply.disabled = Boolean(next.proof);
    visible(qr, Boolean(next.proof)); if (next.proof) qr.src = next.proof.qrImage; else qr.removeAttribute('src');
    visible(job, Boolean(next.job)); job.textContent = next.job ? `迁移状态：${next.job.status}` : ''; visible(refresh, next.canRefresh); refresh.disabled = Boolean(next.proof); visible(cancel, next.canCancel); cancel.disabled = Boolean(next.proof); visible(rollback, next.canRollback && next.rollbackDaysRemaining !== undefined); rollback.textContent = `在 ${next.rollbackDaysRemaining ?? 0} 天内回滚`; rollback.disabled = Boolean(next.proof); back.disabled = busy;
  };
