...
- Cleanup verification: the temporary HTTP server was stopped after browser inspection. Running `$task3Listener = Get-NetTCPConnection -LocalPort 4173 -State Listen -ErrorAction SilentlyContinue; if ($null -eq $task3Listener) { 'PASS: no listener on TCP 4173' } else { $task3Listener | Format-Table -AutoSize; exit 1 }` returned `PASS: no listener on TCP 4173`, confirming that port 4173 was no longer listening.
