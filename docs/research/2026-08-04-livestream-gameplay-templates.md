...
玩法模板与 OBS 外观必须解耦：玩法模板生成属性、礼物规则和定时器；外观主题只改变布局、字体、颜色、描边、阴影和过渡动画。每个玩法可以推荐一个默认主题，但创建后可以随时切换，切换主题不应改变数值或重新生成 OBS 链接。

首批建议提供 6 套主题：

| 主题 | 视觉方向 | 推荐玩法 |
| --- | --- | --- |
| **极简透明** | 无大面积底色，细描边，小阴影，优先保证游戏画面可见 | 计数器、倒计时 |
| **玻璃卡片** | 半透明磨砂、圆角、柔和高光；作为当前样式的标准化版本 | 加班机、目标进度 |
| **RPG Boss** | 厚重血条、金属 / 暗色边框、Boss 名称与当前 / 最大生命 | Boss 挑战 |
| **像素街机** | 像素字体、硬边框、分段进度与跳动数字 | 挑战次数、复活次数 |
| **霓虹科技** | 深色背景、发光描边、双色能量条与扫描过渡 | 资源条、双向拉扯 |
| **软萌应援** | 低对比圆角、明亮强调色、礼物卡片和轻量弹性动画 | 礼物目标、应援进度 |

主题不应保存整段自定义 CSS。建议建立带版本的主题注册表，并让每个属性保存 `displayThemeId` 与少量覆盖项：强调色、面板透明度、字体大小、对齐和是否显示连接状态。找不到旧主题时回退到标准主题，避免升级后 OBS 页面失效。

主题注册表至少需要统一这些设计令牌：

- 面板背景、透明度、描边、圆角和阴影；
- 属性标题、主数值、单位、礼物名与规则名的字号层级；
- 主区域、礼物规则网格和播报区域的间距与密度；
- 进度条 / 血条 / 资源条 / 拉扯条的轨道和填充样式；
- 数值变化、礼物播报切换和目标达成的动画；
- 超长数值、长属性名和长规则名的缩放、换行与截断规则。

外观设置建议放在每个属性的“OBS 输出”工作区：上方使用可点击的主题缩略图，下方只显示当前主题允许覆盖的少量控件，并在右侧持续显示真实比例预览。全局设置可以提供“默认主题”，但每个属性应能独立选择，因为一个 OBS 链接只显示一个属性面板。[当前属性模型](../../src/types.ts) [当前单属性 OBS 面板](../../src/ui/display/display.ts)
