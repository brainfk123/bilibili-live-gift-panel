...
func TestWindowsBundleCommitNoReplacePreservesExistingDirectory(t *testing.T) {
	base := t.TempDir()
	source := filepath.Join(base, "source")
	target := filepath.Join(base, "target")
	if err := createPrivateDirectory(source); err != nil {
		t.Fatal(err)
	}
	if err := writePrivateBundleFile(filepath.Join(source, "policy.json"), []byte("candidate")); err != nil {
		t.Fatal(err)
	}
