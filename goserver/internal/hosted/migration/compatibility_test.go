...
func TestAssessCompatibilityReturnsExactStableContracts(t *testing.T) {
	...
	tests := []struct {
		...
		{"partial reasons are stable when display scenes and crop presets are both unavailable", withCropPresetIDs(withDisplaySceneIDs(baseUnit, "private-scene"), "gift:1"), CapabilitySet{UnitKinds: map[string]bool{"attribute": true}, RulesSupported: true, TimerRulesSupported: true, FormulaPresetsSupported: true}, Compatibility{Status: CompatibilityPartial, ReasonCodes: []string{"crop_presets_unsupported", "display_scenes_unsupported"}}},
